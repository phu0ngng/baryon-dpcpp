...
template<class T> class multi4d
{
    public:
        ...
        explicit multi4d(int ns4, int ns3, int ns2, int ns1) {
            copymem = false;
            F = 0;
            resize(ns4, ns3, ns2, ns1); //request size
        }
        ~multi4d() {
            if (! copymem) {delete[] F;}
        }
        ...
        multi4d(const multi4d& s)
            : copymem(false), n1(s.n1), n2(s.n2), n3(s.n3), n4(s.n4), sz(s.sz), F(0)
        {
            resize(n4,n3,n2,n1);
        ...
        void resize(int ns4, int ns3, int ns2, int ns1) 
        {
            ...
            n1=ns1; n2=ns2; n3=ns3; n4=ns4; sz=n1*n2*n3*n4;
            F = new(std::nothrow) T[sz];
            if( F == 0x0 ) { 
                exit(1);
            }
